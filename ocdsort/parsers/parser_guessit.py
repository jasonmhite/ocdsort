...
from .parser_common import ParsedEntry, ParsedSerie, ParsedVideo, ParsedVideoQuality
...
class GuessRegexpId(Transformer):
    ...
    def process(self, mtree, options=None):
        GuessFinder(
            self.guess_regexps_id,
            None,
            self.log,
            options).process_nodes(
            mtree.unidentified_leaves())

add_transformer(
    'guess_regexp_id = ocdsort.parsers.parser_guessit:GuessRegexpId')
...
class GuessitParsedSerie(GuessitParsedVideo, ParsedSerie):
    ...
    @property
    def episodes(self):
        return len(self._guess_result.get('episodeList', list(filter(lambda x: x is not None, [self.episode]))))
